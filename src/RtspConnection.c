...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, bool expectingPayload, int* error) {
    ...
    *error = -1;
    ...
    sock = connectTcpSocket(&RemoteAddr, RemoteAddrLen, StreamConfig.rtspSetupPort, RTSP_TIMEOUT_SEC);
...
int performRtspHandshake(void) {
    ...
    sprintf(rtspTargetUrl, "rtsp%s://%s:%d", useEnet ? "ru" : "", urlAddr, StreamConfig.rtspSetupPort);
    currentSeqNumber = 1;
    hasSessionId = false;
    ...
    if (useEnet) {
        ...
        enet_address_set_port(&address, StreamConfig.rtspSetupPort);
